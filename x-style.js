// TODO: Check if there’s a more modern approach here. This may already be
//  exactly what we want though.
// TODO: Are the naming prefixes / suffixes consistent?

/*
  Note: if we don't inline the style here, DOMContentLoaded may fire
  before we have a chance to load an external file's contents, which can
  causing an ugly flash of unstyled content.
*/
...
const firstSheetInTarget = Array.from(document.styleSheets).find(
  sheet => sheet.ownerNode && sheet.ownerNode.parentElement === target
);
const beforeNode = firstSheetInTarget ? firstSheetInTarget.ownerNode : null;
target.insertBefore(style, beforeNode);
style.sheet.insertRule(styles);
